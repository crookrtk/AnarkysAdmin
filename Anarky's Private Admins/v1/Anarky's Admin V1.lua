...
local invite = "https://discord.gg/HKwEQ74cJr"
...
if not isfile("AnarkyMoment.lua") then
	writefile("AnarkyMoment.lua", ".")
end
		...
		if _G.chatSpyInstance == instance then
			...
			elseif enabled and (spyOnMyself == true or p ~= player) then
				...
				if hidden and enabled then
					if public then
						saymsg:FireServer((publicItalics and "/me " or '').."{Anarky's admin'} [".. p.Name .."]: "..msg, "All")
					else
						privateProperties.Text = "{Anarky's admin'} [".. p.DisplayName .."]: "..msg
						StarterGui:SetCore("ChatMakeSystemMessage", privateProperties)
					end
				end
			end
...
wait(0.5)
game.ReplicatedStorage.DefaultChatSystemChatEvents.SayMessageRequest:FireServer("I am using Anarky's Private Admin (v1)", "All")
end)
...
for _,x in next, Targets do
    if GetPlayer(x) and GetPlayer(x) ~= Player then
        if GetPlayer(x).UserId ~= 67554151 and GetPlayer(x).UserId ~= 770824 and GetPlayer(x).UserId ~= 22359969 then
            local TPlayer = GetPlayer(x)
            if TPlayer then
                SkidFling(TPlayer)
            end
